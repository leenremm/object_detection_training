...
def xml_to_csv(path):
    ...
    for xml_file in glob.glob(path + "/*.xml"):
        ...
        for member in root.findall("object"):
            ...
            bndbox_key = [i.tag for i in member].index("bndbox")
            value = (
                root.find("filename").text,
                int(root.find("size")[0].text),
                int(root.find("size")[1].text),
                member[0].text,
                int(member[bndbox_key][0].text),
                int(member[bndbox_key][1].text),
                int(member[bndbox_key][2].text),
                int(member[bndbox_key][3].text),
            )
